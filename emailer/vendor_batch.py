...
emailer/message_vendor.txt per lead (business name + the peptides their own site lists),
...
DRAFTS = ROOT / "outreach_drafts" / "vendor"
MESSAGE = ROOT / "emailer" / "message_vendor.txt"
...
CENTRAL_LOG = ROOT / "outreach" / "sent_log.csv"

def already_contacted() -> set:
    done = set()
    for log in (CENTRAL_LOG, LOG):
        if not log.exists():
            continue
        with open(log, newline="", encoding="utf-8") as f:
            for r in csv.DictReader(f):
                if r.get("mode") == "sent" or log == CENTRAL_LOG:
                    done.add(r["email"].lower()); done.add(r["domain"])
    return done
...
def pick_leads(limit: int, source: Path) -> list:
    ...
    cands = []
    for r in rows:
        r.setdefault("business_name", r.get("vendor",""))
        r.setdefault("state", r.get("state_mentioned",""))
        email = (r.get("primary_email") or r.get("email") or "").strip().lower()
        dom = (r.get("website") or ("https://"+r.get("domain","")) ).replace("https://","").replace("http://","").strip("/")
        ...
        hits = int(r.get("peptide_hits") or 0)
        if hits < 2:
            continue
    ...
    for c in cands:
        if c[3] in seen_dom:
            continue
        picked.append(c); seen_dom.add(c[3])
        if len(picked) >= limit:
            break
    return picked
...
def vendor_name(raw: str, dom: str) -> str:
    ...
    if JUNK_NAME.search(n) or len(n.split()) > 5:
        return dom.split("/")[0]
    return n

def build(lead_row: dict, email: str, subject_t: str, body_t: str, attachments=()) -> EmailMessage:
    name = vendor_name(lead_row.get("business_name",""), (lead_row.get("website") or lead_row.get("domain","")).replace("https://","").replace("http://","").strip("/"))
    peps = peptide_phrase(lead_row.get("peptide_terms_found",""))
    subject = subject_t.replace("{business_name}", name)
    body = render(body_t.replace("{business_name}", name))
    msg = EmailMessage()
    msg["Subject"] = subject
...
def main() -> None:
    ...
    ap.add_argument("--message-file", type=Path, default=MESSAGE, help="copy to use (emailer/message_vendor.txt or any file in emailer/medspa/)")
    ...
    args = ap.parse_args()
    source = args.source or Path(sorted(glob.glob(str(ROOT / "exports" / "us_peptide_vendors_with_email_*.csv")))[-1])
